...
- (void) drawRect:(NSRect) dirtyRect {
	if(self.stringSize.width <= self.bounds.size.width || !self.mouseOver) {
	...
	if(self.stringSize.width > self.bounds.size.width) {
		CGFloat startWrapping = -((self.stringSize.width - self.bounds.size.width) + 10);
		if(self.point.x <= startWrapping) {
			NSPoint otherPoint = self.point;
			otherPoint.x = self.point.x + self.stringSize.width + 10;
