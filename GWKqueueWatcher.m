...
#import "GWKqueueWatcher.h"

NSString * const GWKqueueWatcherDidChange = @"GWKqueueWatcherDidChange";

@interface GWKqueueWatcher ()
- (void) kqueueFired;
@end

static void GWKqueueWatcherCallback(CFFileDescriptorRef kqRef, CFOptionFlags callBackTypes, void * info) {
	GWKqueueWatcher * watcher = (__bridge GWKqueueWatcher *)info;
	[watcher kqueueFired];
}

@implementation GWKqueueWatcher
...
- (BOOL) start {
	...
	if(!_kqref) {
		...
		_kqref = CFFileDescriptorCreate(NULL,_kqueueid,true,GWKqueueWatcherCallback,&context);
		if(!_kqref) {
			return FALSE;
		}
...
- (void) kqueueFired {
	if(_kqref) {
		...
		if(eventCount > 0) {
			if(self.delegate && [self.delegate conformsToProtocol:@protocol(GWKqueueWatcherDelegate)]) {
				[self.delegate kqueueWatcherDidChange:self];
			}
			NSNotificationCenter * nfc = [NSNotificationCenter defaultCenter];
			[nfc postNotificationName:GWKqueueWatcherDidChange object:self];
		}
